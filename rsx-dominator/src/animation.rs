use {
    crate::{operations::spawn_future, utils::UnwrapJsExt},
    ...
    futures_util::future::{FutureExt, ready},
    ...
    wasm_bindgen::{JsCast, UnwrapThrowExt, closure::Closure},
    web_sys::window,
};
...
impl Raf {
        ...
        F: FnMut(f64) + 'static,
    {
        ...
        fn schedule(callback: &Closure<dyn FnMut(f64)>) -> i32 {
            window().unwrap_throw().request_animation_frame(callback.as_ref().unchecked_ref()).unwrap_js()
        }
        ...
        *state.borrow_mut() = Some(RafState { id: schedule(&closure), closure });
...
impl Drop for Raf {
    fn drop(&mut self) {
        ...
        window().unwrap_throw().cancel_animation_frame(state.id).unwrap_js();
    }
}
    ...
    F: FnMut(S::Item, AnimatedMapBroadcaster) -> A,
{
    fn animated_state(duration: f64) -> AnimatedMapState {
        let state = AnimatedMapState { animation: MutableAnimation::new(duration), removing: None };
...
pub fn timestamps_difference() -> impl Signal<Item = Option<f64>> {
    ...
    timestamps().map(move |current_time| {
        let diff =
            current_time.map(|current_time| previous_time.map(|previous_time| current_time - previous_time).unwrap_or(0.0));
...
pub struct OnTimestampDiff(#[allow(dead_code)] DiscardOnDrop<CancelableFutureHandle>);
...
impl OnTimestampDiff {
    ...
    where
        F: FnMut(f64) + 'static,
    {
        OnTimestampDiff(spawn_future(timestamps_absolute_difference().for_each(move |diff| {
